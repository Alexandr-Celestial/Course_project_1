...
def test_read_finance_excel_operation() -> None:
    ...
    """
    dates = (
        datetime.datetime(2021, 12, 27, 22, 53, 10),
        datetime.datetime(2021, 12, 1, 22, 53, 10),
    )
    success_test = read_finance_excel_operation(
        dates, ROOT_DIR + "/data/operations.xlsx"
    )
...
def test_currency_rates(requests_mock: Any) -> None:
    ...
    data = currency_rates()
    assert data == [
        {"currency": "USD", "rate": 32.22},
        {"currency": "EUR", "rate": 32.22},
    ]
