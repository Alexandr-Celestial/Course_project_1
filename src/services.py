...
def read_excel_file(file_path: str = ROOT_DIR + "/data/operations.xlsx") -> list[dict]:
...
def rec_search_request(search_word: str | None) -> str:
...
def search_mobile_number_excel() -> str:
    ...
    return json.dumps(data_mobile_number)


def search_transactions_to_people() -> str:
    """Функция для поиска переводов физ лицам"""
    sent_transaction = []
    excel_data = read_excel_file()
    for data in excel_data:
        re_search = re.search(
            r"^[А-ЯЁ][а-яё]+\s+[А-ЯЁ]\.$", str(data["Категория"]), re.I
        )
