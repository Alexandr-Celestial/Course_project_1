...
file_handler = logging.FileHandler(
    ROOT_DIR + "/logs/logging_utils.log", mode="w", encoding="utf-8"
)
...
def read_finance_excel_operation(
    period_datetime: tuple[datetime, datetime],
    filename: str | None = ROOT_DIR + "/data/operations.xlsx",
) -> list[dict[Any, Any]]:
    """
    ...
    if filename:
        try:
            ...
            filtered_data = [
                ...
                if datetime.strptime(data["Дата операции"], "%d.%m.%Y %H:%M:%S")
                >= start_date
                and datetime.strptime(data["Дата операции"], "%d.%m.%Y %H:%M:%S")
                <= end_date
            ]
            utils_logger.info(
                "Данные по файлу транзакций отфильтрован по дате и готов к работе"
            )
            return list(filtered_data)
        except Exception:
            utils_logger.error(
                "Произошла ошибка в чтении файла и/или в преобразовании ячейки в формат даты"
            )
            raise Exception(
                "Произошла ошибка в чтении файла и/или в преобразовании ячейки в формат даты"
            )
...
def main_cards(transactions: list[dict]) -> list[dict]:
    ...
    try:
        ...
        add_group_data = df.groupby("Номер карты").agg(
            {"Сумма операции с округлением": "sum", "Кэшбэк": "sum"}
        )
...
def get_api_currency(currency: str) -> float:
    ...
    try:
        ...
        raise requests.exceptions.ReadTimeout(
            "Превышено время соединения с сервером API"
        )
